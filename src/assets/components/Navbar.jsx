      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative cursor-pointer group hidden sm:block">
          ...
          <div className="absolute right-0 mt-4 w-56 bg-[#111] border border-gray-800 rounded-xl p-3 opacity-0 group-hover:opacity-100 transition pointer-events-none group-hover:pointer-events-auto z-50">
            <p className="text-sm mb-1">🔥 New drops available</p>
            <p className="text-sm mb-1">⚡ -20% on Nike</p>
            <p className="text-sm">👟 New arrivals this week</p>
          </div>
        </div>

        {/* Корзина */}
        <button onClick={onCartClick} className="relative cursor-pointer text-xl">
          🛒
          {cartCount > 0 && (
            <span className="absolute -top-2 -right-3 bg-lime-400 text-black text-xs w-5 h-5 flex items-center justify-center rounded-full">
              {cartCount}
            </span>
          )}
        </button>
